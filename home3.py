...
from newsapi.newsapi_client import NewsApiClient  #library
...
def home():
	...
	while True:
            ...
            return render_template('home2.html',contents=contents,coba=coba,url=url,greet=greet, temp_c=temp_c, humi=humi)
...
def gen():
    while True:
        ...
        if len(a) == 0:
            id="unknown"
            name="maaf kami tidak bisa mengenali anda"
            inistatus="unknown"
        elif len(a) ==1:
            id = a[0]
            face=Users.query.filter_by(id=id).one()
            name=str(face.name)
            inistatus=face.status
        
        f = open("/home/pi/smartmirror3/sample.txt", "w")
        f.write(name)
        f.close()
...
def faceid():
    with open("sample.txt", "r") as inp:  #Read phase
        data = inp.readlines()
        name = data
        inistatuss = "status"
        return jsonify({'result' : 'success', 'name' : name, 'inistatus' : inistatuss})
