...
def gen():
    while True:
        ...
        #print(str(id) + ' ' + name + ' ' + inistatus)
        #session['id'] = id
        #time taken before the user is logged off from the mirror
        cv2.imwrite('pic.jpg', img)
        yield (b'--img\r\n' 
              b'Content-Type: image/jpeg\r\n\r\n' + open('pic.jpg', 'rb').read() + b'\r\n')
        #yield{'name': name, 'inistatus': inistatus}
   
#from operator import itemgetter
#all_values = [(images,names) for images,names in gen()]
#names = map(itemgetter(1), all_values)
#images = process_images(map(itemgetter(0), all_values))
...
def faceid():
    while True:
        #iniid = request.cookies.get('id')
        #print(iniid)
        names = "ininamahahahah"
        inistatuss = "inistatus"
        return jsonify({'result' : 'success', 'name' : names, 'inistatus' : inistatuss})
